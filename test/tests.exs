...
defmodule SchemaIrveTest do
  ...
  defp tarif_field_example!(name) do
    read_json!("tarifs/schema-tarifs.json")
    |> Map.fetch!("fields")
  ...
  test "price-components is a valid JSON Schema" do
    assert %ExJsonSchema.Schema.Root{} = resolve_schema!("tarifs/price-components.schema.json")
  end

  test "restrictions example follows its own schema" do
    schema = resolve_schema!("tarifs/restrictions.schema.json")
  ...
  test "price_components example follows its own schema" do
    schema = resolve_schema!("tarifs/price-components.schema.json")
